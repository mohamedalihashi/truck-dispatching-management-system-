...
export function DriverDashboardCharts({
  statusData,
  trendData,
  earningsData,
  totalTrips,
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-12">
        ...
        <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
          <div>
            <h2 className="text-xl font-semibold text-primary-container">
              {isShared ? "Shared trip activity" : "Your trip activity"}
            </h2>
            <p className="mt-1 text-sm text-on-surface-variant">
              {isShared
                ? "Published loads and deliveries over the last six months."
                : "Assigned jobs and deliveries over the last six months."}
            </p>
          </div>
        ...
        <div className="h-72">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={trendData?.length ? trendData : [{ name: "—", trips: 0, delivered: 0 }]}>
              <defs>
                <linearGradient id="drvTripsFill" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="#fe6b00" stopOpacity={0.35} />
                  <stop offset="100%" stopColor="#fe6b00" stopOpacity={0.04} />
                </linearGradient>
                <linearGradient id="drvDeliveredFill" x1="0" y1="0" x2="0" y2="1">
      ...
      <section className="rounded-xl border border-outline-variant bg-surface-container-lowest p-6 shadow-[0px_4px_20px_rgba(0,0,0,0.05)] xl:col-span-5">
        <div>
          <h2 className="text-xl font-semibold text-primary-container">
            {isShared ? "Load pipeline" : "Job status"}
          </h2>
          <p className="mt-1 text-sm text-on-surface-variant">
            {isShared ? "Where your shared loads stand right now." : "Breakdown of your recent jobs."}
          </p>
        </div>
        <div className="relative mt-3 h-56">
          ...
          <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
            ...
            <span className="text-xs text-on-surface-variant">{isShared ? "Total loads" : "Recent jobs"}</span>
          </div>
        </div>
      ...
      {!isShared && earningsData ? (
        <section className="overflow-hidden rounded-xl border border-outline-variant bg-surface-container-lowest p-6 shadow-[0px_4px_20px_rgba(0,0,0,0.05)] xl:col-span-12">
          <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
            <div>
              <h2 className="text-xl font-semibold text-primary-container">Earnings snapshot</h2>
              <p className="mt-1 text-sm text-on-surface-variant">Available balance vs paid out.</p>
            </div>
          </div>
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={earningsData} layout="vertical" margin={{ left: 16, right: 24 }}>
                <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="#c5c6cd" />
                <XAxis type="number" stroke="#75777e" />
                <YAxis type="category" dataKey="name" width={88} stroke="#75777e" />
